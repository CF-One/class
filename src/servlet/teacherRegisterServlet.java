...
public class teacherRegisterServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //调用userDao中的注册用户方法
        //判断用户手机号是否注册
        if (userTeacherDao.queryOneUserPhone(phone)||userDao.queryOneUserPhone(phone)){
            //手机号存在
            ...
            request.getRequestDispatcher("/jsp/registerTeacher.jsp").forward(request, response);
        }else{
            //手机号不存在允许注册
                //该学号未注册
                if(userTeacherDao.register(user)){
                    ...
                    response.sendRedirect(request.getContextPath()+"/jsp/registerTeacher.jsp");
